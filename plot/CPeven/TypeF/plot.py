...
data=np.loadtxt("CPeven_TypeF.txt")
mC=data[:,0]
tanb=data[:,1]
length=data[:,2]
gammalength=data[:,3]
...
contours1=plt.contour(mC,tanb,length,[0.0001,0.01, 1,10],colors='r')
#contours2=plt.contourf(mC,tanb,gammalength,[102,13081.7],colors='0.8')
#plt.clabel(contours1, inline=False, fontsize=16,fmt='%1.3f')

fmt = {}
strs = [r'$10^{-4}$',r'$10^{-2}$',r'1',r'10']
for l, s in zip(contours1.levels, strs):
    fmt[l] = s
manual_locations = [(0.3, 20), (0.1,10), (0.03, 8), (0.015,5)]
labels1=plt.clabel(contours1, contours1.levels, inline=False,inline_spacing=0, fmt=fmt,manual=manual_locations, fontsize=16)
...
plt.title(r'Type-F: $\cos(\beta-\alpha)=1/\tan\beta$',fontsize=18)
